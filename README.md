# Android Monitor Console

> 안드로이드 폰을 활용한 서버 상태 모니터링 콘솔
> 유휴 모바일 디바이스를 실시간 운영 패널로 재활용하는 프로젝트

---

## 프로젝트 개요

Android Monitor Console은 안드로이드 폰을 책상 위에 고정하여   
서버 상태를 실시간으로 확인하는 **모바일 전용 모니터링 대시보드**다.

이 프로젝트는 단순한 웹 대시보드가 아니라   
**항상 켜져 있는 운영 콘솔 형태의 UI**를 목표로 한다.

샤오미 14T를 전용 실행 디바이스로 사용하며   
Termux 기반 로컬 서버 + 브라우저 UI 구조로 동작한다.
---

## 목표

- 유휴 안드로이드 디바이스를 운영 패널로 재활용
- 서버 상태를 실시간으로 시각화
- CLI 스타일의 간결한 UI 제공
- 터치 기반 관리 인터페이스 제공
- 로컬 환경에서 독립적으로 실행 가능

---

## 주요 기능

### 1. **디바이스 상태 반영**

- 배터리
- 충전 여부
- 화면 활성 상태
- 유휴 시간
- 네트워크 상태

### 2. **외부 환경 반영**

- 날씨
- 미세먼지
- 시간대

---

## 왜 샤오미 14T인가

이 프로젝트에서 Xiaomi 14T는 개발 장비가 아니라 **전용 실행 단말**로 사용된다.

역할:
- 상시 실행 모니터링 패널
- 터치 기반 관리 인터페이스
- 실시간 상태 표시 화면

PC에서 개발한 뒤 안드로이드 폰으로 배포하는 구조로 사용한다.

---

## 시스템 구조

```text
[외부 API / 로컬 데이터]
- 날씨 / 미세먼지
- 디바이스 상태

        ↓

[Flask Backend]
- 상태 해석 엔진
- 시간대 판단
- 유휴 상태 판단

        ↓

[Frontend UI]
- 모바일 최적화 UI
- 상태 표시
- 실시간 갱신
```

## 기술 스택

### Runtime

| 구성 요소 | 설명 |
| --- | --- |
| Android | 실행 디바이스 (샤오미 14T) |
| Termux | Android 내 Linux 실행 환경 |
| Python | 백엔드 로직 구현 |
| Flask | 상태 처리 및 API 서버 |

### Frontend

| 구성 요소 | 설명 |
| --- | --- |
| HTML | UI 구조 |
| CSS | 스타일링 |
| JavaScript | 상태 업데이트 및 API 호출 |

### External API

| API | 용도 |
| --- | --- |
| 날씨 API | 현재 날씨 정보 |
| 대기질 API | 미세먼지/공기 상태 |

---

## 상태 시스템

### 기본 상태

| 상태 | 의미 |
| --- | --- |
| idle | 기본 대기 상태 |
| sleep | 유휴 상태 (장시간 입력 없음) |
| focus | 사용 중 / 집중 상태 |
| warning | 경고 상태 (배터리, 환경 등) |
| healing | 회복 상태 (충전 등) |
| event | 이벤트 발생 상태 |

### 상태 전환 규칙

| 조건 | 상태 |
| --- | --- |
| 일정 시간 이상 입력 없음 | sleep |
| 배터리 낮음 | warning |
| 충전 중 | healing |
| 일정 또는 할 일 발생 | event |
| 사용자 활동 중 | idle / focus |

---

## 디렉토리 구조

```
android-monitor-console/
├── app/
│   ├── api/
│   ├── core/
│   ├── constants/
│   ├── services/
│   ├── repositories/
│   ├── models/
│   └── main.py
├── web/
│   ├── static/
│   │   ├── css/
│   │   ├── js/
│   │   └── img/
│   └── templates/
│       └── index.html
├── data/
│   └── messages/
├── scripts/
├── requirements.txt
└── README.md
```

---
## 실행 방법

### [1단계] Windows에서 Flask 서버 실행 확인

#### 1. 가상환경 생성

```
python -m venv venv
```

#### 2. 가상환경 활성화

```
venv\Scripts\activate
```

#### 3. 패키지 설치

```
pip install -r requirements.txt
```

#### 4. 설치 확인

```
pip list
```

#### 5. 서버 실행

```
python -m app.main
```
`app.main` 내부에서 Flask 앱을 직접 실행하므로 `python -m app.main` 방식으로 기동한다

#### 6. 접속 확인

```
http://127.0.0.1:8000
```

### [2단계] Termux 배포 및 실행

#### 1. Termux 기본 설정
```bash
pkg update && pkg upgrade -y
pkg install python git termux-api -y
pkg install termux-api
termux-setup-storage
```
#### 2. 프로젝트 복사
```bash
git clone https://github.com/seoheejung/android-monitor-console.git ~/android-monitor-console
cd ~/android-monitor-console

touch app/__init__.py
touch app/services/__init__.py
touch app/repositories/__init__.py
touch app/models/__init__.py
```

#### 3. 실행 권한 부여
```bash
chmod +x termux_setup.sh
chmod +x termux_start.sh
```

#### 4. 의존성 설치
```bash
./termux_setup.sh
```

#### 5. 서버 실행

```bash
./termux_start.sh
```
#### 6. 브라우저 접속

```bash
http://127.0.0.1:8000
```

#### 7. 모바일 패널 사용

- 브라우저 접속
- 전체화면 모드
- 충전 상태 유지
- 화면 꺼짐 시간은 Android 설정에서 별도 조정

#### 8. 종료 방법

Termux 실행 화면에서:

```bash
Ctrl+ C
```

---

### [3단계] Termux 재실행 절차

#### 1. Termux를 다시 열었을 때
```
cd ~/android-monitor-console
./termux_start.sh
```

#### 2. 브라우저에서 다시 접속
```
http://127.0.0.1:8000
```

---

### ⚠️ 환경별 의존성 파일
| 파일                        | 용도               |
| ------------------------- | ---------------- |
| `requirements.txt`        | Windows / 로컬 개발용 |
| `requirements-termux.txt` | Termux 실행용       |


### ⚠️ 환경별 배터리 처리 방식
| 환경              | 방식                    |
| --------------- | --------------------- |
| Windows / Linux | psutil                |
| Termux          | termux-battery-status |
> psutil은 Termux에서 사용 불가

### ⚠️ 주요 주의사항
| 항목            | 내용                           |
| ------------- | ---------------------------- |
| 실행 위치         | 반드시 프로젝트 루트에서 실행             |
| 포트            | 기본 8000 사용                   |
| Termux 배터리 조회 | `termux-api` 필요              |
| 로컬 개발         | `requirements.txt` 사용        |
| Termux 실행     | `requirements-termux.txt` 사용 |
| 네트워크 접속       | PC에서 폰에 접근할 경우 동일 Wi-Fi 필요   |

---

## 실행 흐름

1. 앱 실행
2. 디바이스 상태 수집
3. 외부 API 데이터 조회
4. 상태 해석 엔진이 현재 상태 계산
5. UI에서 주기적 갱신
6. 사용자 입력 처리

---

## 개발 원칙

- 모바일 전용 UI 기준 설계
- 최소한의 정보로 상태 전달
- 실시간성 우선
- 불필요한 장식 제거
- 운영 도구로서의 안정성 유지

---

## 프로젝트 상태

진행 중 (Monitoring UI 개선 단계)

---

## 라이선스

MIT

---